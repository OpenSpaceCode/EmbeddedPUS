...
static pus_status_t getter_batt_alert(uint16_t pid, uint8_t *buf,
                                       uint16_t cap, void *ud)
{
...
static pus_status_t setter_batt_alert(uint16_t pid, const uint8_t *buf,
...
static pus_status_t hk_power_provider(uint16_t sid, uint8_t *buf,
...
static pus_status_t hk_thermal_provider(uint16_t sid, uint8_t *buf,
                                         uint16_t cap, uint16_t *out_len,
                                         void *ud)
{
    (void)sid; (void)cap; (void)ud;
...
static void pus_app_init(void)
{
    ...
    pus_service_20_register_param(&g_s20, PID_BATT_ALERT_MV, 2u,
                                  getter_batt_alert, setter_batt_alert, NULL);
    pus_service_20_register_handlers(&g_pus, &g_s20);

    /* Announce nominal mode via ST[05] */
    pus_service_5_emit(&g_pus, PUS_SUBTYPE_EVENT_INFO,
                       EVID_MODE_NOMINAL, NULL, 0u);
}
...
static void pus_app_send_housekeeping(void)
{
    ...
    if (platform_read_battery_mv() < g_batt_alert_mv) {
        ...
        pus_service_5_emit(&g_pus, PUS_SUBTYPE_EVENT_HIGH_SEVERITY,
                           EVID_BATTERY_LOW, aux, sizeof(aux));
    }
}
...
int main(void)
{
    ...
    static const uint8_t tc_17_1[] = {
        ...
        17u,         /* service */
        1u,          /* subtype */
        0x00, 0x10,  /* source_id = 0x0010 */
        0x00, 0x00, 0x00, 0x00, 0x00  /* time + spare */
    };
