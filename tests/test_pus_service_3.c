...
static int test_register_hk_emit(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_3_register_hk(&s3, 0x0001u, simple_provider, NULL));
...
static int test_register_diag_emit(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_3_emit_diag(&ctx, &s3, 0x0002u));
...
static int test_unknown_sid_returns_no_handler(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_NO_HANDLER, pus_service_3_emit_hk(&ctx, &s3, 0xFFFFu));
...
static int test_table_full(void)
{
    ...
    for (uint8_t i = 0u; i < PUS_SERVICE_3_MAX_STRUCTURES; i++)
    {
        ASSERT_EQ_INT(PUS_STATUS_OK,
                      pus_service_3_register_hk(&s3, (uint16_t)i, simple_provider, NULL));
    }
...
{
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_service_3_register_hk(NULL, 0x0001u, simple_provider, NULL));
    return 0;
}
...
static int test_register_null_provider(void)
{
    pus_service_3_ctx_t s3;
    pus_service_3_init(&s3);
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_service_3_register_hk(&s3, 0x0001u, NULL, NULL));
    return 0;
}
...
static int test_provider_failure(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_HANDLER_FAILED, pus_service_3_emit_hk(&ctx, &s3, 0x0001u));
    return 0;
}
...
static int test_provider_data_overflow(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_BAD_LENGTH, pus_service_3_emit_hk(&ctx, &s3, 0x0001u));
    return 0;
}
