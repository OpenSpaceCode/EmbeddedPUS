...
static int test_build_success_payload(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK,
                  pus_service_1_build_success(&ctx,
                                              &tc,
...
static int test_build_failure_payload(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK,
                  pus_service_1_build_failure(&ctx,
                                              &tc,
...
static int test_emit_no_sink_ok(void)
{
    ...
    ASSERT_EQ_INT(
        PUS_STATUS_OK,
        pus_service_1_emit_failure(&ctx, &tc, PUS_SUBTYPE_VERIFICATION_ROUTING_FAILURE, 0u));
...
static int test_build_null_args(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_NULL,
                  pus_service_1_build_success(NULL,
                                              &tc,
                                              PUS_SUBTYPE_VERIFICATION_ACCEPTANCE_SUCCESS,
                                              out,
                                              sizeof(out),
...
static int test_emit_null_ctx(void)
{
    ...
    ASSERT_EQ_INT(
        PUS_STATUS_NULL,
        pus_service_1_emit_success(NULL, &tc, PUS_SUBTYPE_VERIFICATION_ACCEPTANCE_SUCCESS));
    ASSERT_EQ_INT(
        PUS_STATUS_NULL,
        pus_service_1_emit_failure(NULL, &tc, PUS_SUBTYPE_VERIFICATION_ROUTING_FAILURE, 0u));
    return 0;
}
