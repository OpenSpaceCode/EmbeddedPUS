...
static int test_emit_alive_report(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_17_emit_alive_report(&ctx, 0x0042u));
...
static int test_emit_connection_report(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_17_emit_connection_report(&ctx, 0xBEEFu, 0x0001u));
...
{
    pus_context_t ctx = make_ctx();
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_17_register_handlers(&ctx));
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK,
                  pus_handler_invoke(&ctx, PUS_SERVICE_TEST, PUS_SUBTYPE_TEST_ARE_YOU_ALIVE, &tc));
...
static int test_handler_17_3_echoes_apid(void)
{
    ...
    ASSERT_EQ_INT(
        PUS_STATUS_OK,
        pus_handler_invoke(&ctx, PUS_SERVICE_TEST, PUS_SUBTYPE_TEST_ON_BOARD_CONNECTION, &tc));
...
{
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_service_17_emit_alive_report(NULL, 0u));
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_service_17_emit_connection_report(NULL, 0u, 0u));
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_service_17_register_handlers(NULL));
    return 0;
}
