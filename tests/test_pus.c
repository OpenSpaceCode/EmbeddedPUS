...
static int test_tc_process_all_acks_success(void)
{
    ...
    uint8_t buf[PUS_TC_SEC_HEADER_LEN];
    make_tc_buf(buf, 17u, 1u, (uint8_t)(PUS_ACK_ACCEPTANCE | PUS_ACK_START | PUS_ACK_COMPLETION));
 ...
 */
static pus_status_t progress_handler(pus_context_t *ctx, const pus_tc_packet_t *tc, void *ud)
{
    (void)ud;
    if (tc->sec_header.ack_flags & PUS_ACK_PROGRESS)
    {
        pus_service_1_emit_success(ctx, tc, PUS_SUBTYPE_VERIFICATION_PROGRESS_SUCCESS);
    }
    return PUS_STATUS_OK;
}
...
static int test_tm_build_capacity_too_small(void)
{
    ...
    uint8_t payload[8] = {0};
    ASSERT_EQ_INT(PUS_STATUS_BUFFER_TOO_SMALL,
                  pus_tm_build(&ctx, 1u, 1u, 0u, payload, sizeof(payload), buf, sizeof(buf), &len));
    return 0;
}
...
static int test_tm_build_null_payload_zero_fills(void)
{
    ...
    uint8_t buf[64];
    uint16_t len;
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_tm_build(&ctx, 17u, 2u, 0u, NULL, 4u, buf, sizeof(buf), &len));
...
static int test_tm_build_no_sink(void)
{
    ...
    uint8_t buf[64];
    uint16_t len;
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_tm_build(&ctx, 17u, 2u, 0u, NULL, 0u, buf, sizeof(buf), &len));
