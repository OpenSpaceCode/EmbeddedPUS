...
static pus_status_t param_setter(uint16_t pid, const uint8_t *buf, uint16_t len, void *ud)
{
    ...
    (void)len;
    (void)ud;
    g_param_val = ((uint32_t)buf[0] << 24u) | ((uint32_t)buf[1] << 16u) | ((uint32_t)buf[2] << 8u) |
                  (uint32_t)buf[3];
    return PUS_STATUS_OK;
}
...
static int test_register_and_emit_single(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_20_emit_report(&ctx, &s20, ids, 1u));
...
static int test_emit_multiple_params(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_20_emit_report(&ctx, &s20, ids, 2u));
...
static int test_set_values_tc(void)
{
    ...
    pus_service_20_init(&s20);
    pus_service_20_register_param(&s20, 0x0010u, 4u, param_getter, param_setter, NULL);
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_20_register_handlers(&ctx, &s20));
...
static int test_readonly_param_returns_failed(void)
{
    ...
    pus_service_20_register_param(&s20, 0x0020u, 4u, param_getter, NULL, NULL);
...
static int test_unknown_param_returns_no_handler(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_NO_HANDLER, pus_service_20_emit_report(&ctx, &s20, ids, 1u));
...
static int test_register_table_full(void)
{
    ...
    for (uint8_t i = 0u; i < PUS_SERVICE_20_MAX_PARAMS; i++)
    {
        ASSERT_EQ_INT(
            PUS_STATUS_OK,
            pus_service_20_register_param(&s20, (uint16_t)i, 4u, param_getter, NULL, NULL));
    }
...
static int test_emit_report_null(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_service_20_emit_report(NULL, &s20, ids, 1u));
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_service_20_emit_report(&ctx, NULL, ids, 1u));
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_service_20_emit_report(&ctx, &s20, NULL, 1u));
    return 0;
}
...
static int test_emit_report_getter_failure(void)
{
    ...
    const uint16_t ids[] = {0x0001u};
    ASSERT_EQ_INT(PUS_STATUS_HANDLER_FAILED, pus_service_20_emit_report(&ctx, &s20, ids, 1u));
    return 0;
}
...
static int test_emit_report_buffer_too_small(void)
{
    ...
    const uint16_t ids[] = {0x0001u};
    ASSERT_EQ_INT(PUS_STATUS_BUFFER_TOO_SMALL, pus_service_20_emit_report(&ctx, &s20, ids, 1u));
    return 0;
}
...
static int test_tc_20_3_setter_failure(void)
{
    ...
    pus_service_20_ctx_t s20;
    pus_service_20_init(&s20);
    pus_service_20_register_param(&s20, 0x0010u, 4u, param_getter, failing_setter, NULL);
    pus_service_20_register_handlers(&ctx, &s20);
...
static int test_emit_report_count_zero(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_service_20_emit_report(&ctx, &s20, ids, 0u));
