...
static int test_tc_encode(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_tc_sec_header_encode(&hdr, out, sizeof(out), &len));
...
static int test_tc_decode(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_tc_sec_header_decode(raw, sizeof(raw), &hdr));
...
static int test_tc_roundtrip(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_tc_sec_header_encode(&orig, buf, sizeof(buf), &len));
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_tc_sec_header_decode(buf, len, &decoded));
...
static int test_tm_encode(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_tm_sec_header_encode(&hdr, out, sizeof(out), &len));
...
static int test_tm_decode(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_tm_sec_header_decode(raw, sizeof(raw), &hdr));
...
static int test_buffer_too_small(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_BAD_LENGTH, pus_tc_sec_header_decode(small, sizeof(small), &tc_hdr));
    ASSERT_EQ_INT(PUS_STATUS_BUFFER_TOO_SMALL,
                  pus_tc_sec_header_encode(&tc_hdr, small, sizeof(small), &len));

    ASSERT_EQ_INT(PUS_STATUS_BAD_LENGTH, pus_tm_sec_header_decode(small, sizeof(small), &tm_hdr));
    ASSERT_EQ_INT(PUS_STATUS_BUFFER_TOO_SMALL,
...
static int test_bad_version(void)
{
    ...
    pus_tc_sec_header_t hdr;
    ASSERT_EQ_INT(PUS_STATUS_BAD_VERSION, pus_tc_sec_header_decode(buf, sizeof(buf), &hdr));
    return 0;
}
