...
static int test_register_and_find(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_handler_register(&ctx, 17u, 1u, dummy_handler, NULL));

    /* Invocation returns whatever dummy_handler returns (OK) */
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_handler_invoke(&ctx, 17u, 1u, &tc));
...
static int test_not_found(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_NO_HANDLER, pus_handler_invoke(&ctx, 17u, 1u, &tc));
...
static int test_update_on_duplicate(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_handler_register(&ctx, 17u, 1u, other_handler, NULL));

    /* After update, other_handler (returns HANDLER_FAILED) must be active */
    ASSERT_EQ_INT(PUS_STATUS_HANDLER_FAILED, pus_handler_invoke(&ctx, 17u, 1u, &tc));
...
static int test_multiple_services(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_NO_HANDLER, pus_handler_invoke(&ctx, 20u, 3u, &tc));
...
static int test_table_full(void)
{
    ...
    {
        ASSERT_EQ_INT(PUS_STATUS_OK, pus_handler_register(&ctx, 1u, i, dummy_handler, NULL));
    }
...
static int test_null_context(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_handler_register(NULL, 1u, 1u, dummy_handler, NULL));
    ASSERT_EQ_INT(PUS_STATUS_NULL, pus_handler_invoke(NULL, 1u, 1u, &tc));
...
static int test_deregister_with_null_handler(void)
{
    ...
    ASSERT_EQ_INT(PUS_STATUS_NO_HANDLER, pus_handler_register(&ctx, 17u, 1u, NULL, NULL));

    /* Register, then deregister */
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_handler_register(&ctx, 17u, 1u, dummy_handler, NULL));
    ASSERT_EQ_INT(PUS_STATUS_OK, pus_handler_register(&ctx, 17u, 1u, NULL, NULL));
