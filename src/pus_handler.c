                                  ...
                                  void            *user_data)
{
    ...
    if (handler == NULL)
    {
        ...
        {
            if (ctx->handler_table[i].is_used && ctx->handler_table[i].service == service &&
...
int pus_handler_find(const pus_context_t *ctx, pus_service_t service, pus_subtype_t subtype)
{
    if (ctx == NULL)
    {
        return -1;
                                ...
                                const pus_tc_packet_t *tc)
{
    ...
    return ctx->handler_table[idx].handler(ctx, tc, ctx->handler_table[idx].user_data);
}
